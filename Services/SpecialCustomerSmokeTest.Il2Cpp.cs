...
internal sealed class SpecialCustomerSmokeTest
{
    ...
    public static SpecialCustomerSmokeTest? TryCreate()
    {
        ...
        var outputDirectory = Path.Combine(Path.GetTempPath(), "S1SpecialCustomers.Smoke.Il2Cpp");
        var saveSlot = 1;
        ...
        MelonLogger.Msg($"[S1SpecialCustomersSmoke] Enabled for IL2CPP save slot {saveSlot}.");
    ...
    {
        var result = $"{(passed ? "PASS" : "FAIL")}|Backend=Il2Cpp|Reason={reason}";
